...
import { HOSE_PIPES_MATERIALS } from "../../data/hosePipesData";
import ProductHeroSection from "./ProductHeroSection";

export default function HosePipesSection({ onSelectMaterial, onBackToProducts }) {
  ...
  useEffect(() => {
    document.title = "Hose Pipes | Supreme Metal & Alloys";
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, []);

  // GSAP Entrance Animations matching CategoryPage
  useEffect(() => {
  ...
  // Filter Hose Pipes products by selectedMaterialTab & searchQuery
  const filteredProducts = useMemo(() => {
    return HOSE_PIPES_MATERIALS.filter((product) => {
  ...
  const handleProductClick = (product) => {
    if (onSelectMaterial) {
      onSelectMaterial("hose-pipes", product.slug);
    } else {
      window.location.hash = `#hose-pipes/${product.slug}-manufactured-in-mumbai`;
    }
  };
    ...
    <div ref={containerRef} className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased pb-24">
      ...
      <ProductHeroSection
        category={{
          ...
          fullDesc: "Flexible corrugated metal hoses with single/double layer braiding for vibration and thermal expansion across extreme pressure and temperature environments."
        }}
        totalProducts={HOSE_PIPES_MATERIALS.length}
        materialsCount={HOSE_PIPES_MATERIALS.length}
      ...
      {/* ========================================================================= */}
      {/* 2. MATERIALS AVAILABLE IN HOSE PIPES SECTION */}
      {/* ========================================================================= */}
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-28 rounded-xl bg-white border border-slate-200/80 p-2 mb-3 flex items-center justify-center overflow-hidden group-hover:bg-slate-50 transition-colors">
                    <img
                      src={mat.image}
                      alt={`${mat.materialName} Hose Pipes`}
        ...
        {filteredProducts.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center max-w-xl mx-auto shadow-sm my-8">
            ...
            <p className="text-sm text-slate-600 mb-6 font-medium">
              No Hose Pipes found for the selected material filter. Switch tab to "ALL" to view all Hose Pipes products.
            </p>
            <button
              onClick={() => {
                setSelectedMaterialTab("all");
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <div className="bg-gradient-to-r from-[#0E2A3A] via-[#1e3a4c] to-[#0E2A3A] rounded-2xl p-6 sm:p-8 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-lg border border-slate-700">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-lg font-bold mb-1">Custom Hose Pipe Assembly & End Fitting Services</h4>
              <p className="text-xs sm:text-sm text-slate-300 font-medium">
                Need custom length, flanged, threaded, or quick-cam hose pipe assemblies? Submit your engineering drawings for custom factory assembly.
              </p>
            </div>
          </div>
          <a
